...
[AGRO compatibility contract](https://github.com/mifunedev/agro/blob/main/docs/agro-compatibility.md) defines. The one verb whose
...
| `get-agro.sh` | a plain file | downloads `AGRO_JS_URL` (falls back to `OH_JS_URL`; default `https://agro.mifune.dev/agro.js`) into the same directory, checks its shebang and `--version`, renames it over the executable, and keeps `<path>.prev` until the new file verifies |
...
## The cron scheduler is a systemd service, not an `agro` verb

`agro` owns the container's lifecycle; `systemd` owns the processes inside it. The
scheduler has no `agro` verb — reach it from inside the sandbox:
...
what `agro ps` reports on through the container healthcheck.
